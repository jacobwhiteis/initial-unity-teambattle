...
- **Match format (BO3/BO5):** Determined by the higher-ranked team's position in `POS_RULES` (see `src/crp.js`). Positions 1–5 (Elite) play BO5; positions 6+ play BO3. Ban/pick and battle pages adapt phase logic, map counts, and win thresholds accordingly. Decider map only revealed when series is tied (1-1 for BO3, 2-2 for BO5).
- **Challenge/decline rule:** A team can challenge any team ranked above them within the same division (Elite 1–5, Division 1 6–10, Division 2 11–15, Division 3 16+). Cross-division challenges are not allowed. On decline, position shifts mirror an upset win: challenger jumps to the decliner's slot, decliner and every team between shift down by 1. See `canChallenge` in `src/crp.js`.
